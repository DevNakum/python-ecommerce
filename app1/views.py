...
def productall(request):
    if 'email' in request.session:
        ...
        if request.method == 'POST':
            ...
            dataName=Product.objects.filter(name__icontains=srch) 
            dataDesc=Product.objects.filter(description__icontains=srch)

            finalData = dataName or dataDesc
            return render(request,'productall.html',{'data':finalData,'a':a})
        ...
        if request.method == 'POST':
            ...
            finalData = dataName or dataDesc
            return render(request,'productall.html',{'data':finalData})
        data=Product.objects.all()
...
def buynow(request):
    if 'email' in request.session:
        ...
        if request.method=="POST":
            ...
            productData = Product.objects.get(id=request.POST['id'])
            if int(request.POST['qty1']) <= int(productData.quantity):
                request.session['productid']=request.POST['id']
                request.session['quantity']=request.POST['qty1']
                ...
                request.session['orderAmount']=int(b.price)*int(request.session['quantity'])
                request.session['paymentMethod']="Razorpay"
                request.session['transactionId']=""
                return redirect('razorpayView') 
            else:
                return HttpResponse("<h1>quantity is mroe than actual quantity</h1>")
    else:
        return redirect('login1')


RAZOR_KEY_ID = 'rzp_test_mUYPZJhg6FYh4U'
RAZOR_KEY_SECRET = 'PNlmecc9ugQKwWOWZqb6yuTI'
...
def myOrder(request):
    if 'email' in request.session:
        ...
        data_list = []
        for i in data:
            data2 = Product.objects.get(id = i.productid)
            data_list.append({'data1':i,'data2':data2})
            
        return render(request,'myorder.html',{'data':data_list,'a':a}) 
    else:
        return  redirect('login1')    